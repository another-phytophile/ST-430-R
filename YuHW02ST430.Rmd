...
The point estimate for the slope is `r sactgpam[[4]][1,1]` and the  95% confidence interval for the y-intercept is `r confint(actgpam,level = 0.95)[1,1]` to `r confint(actgpam,level = 0.95)[1,2]`. This indicates that 95% of intervals from samples of the same size will contain $\beta_0$.
...
pv <- as.data.frame(do.call(rbind, list(
  ...
  c("lower tail, 0.00",
    ...
    "as p >0.05, we fail to reject H0 that beta1 = 0 at alpha=0.05"),
  c("upper tail, 0.00",
  ...
  c("upper tail, 0.07",
    ...
    "as p >0.05, we fail to reject H0 that beta1 = 0.07 at alpha=0.05")
)))
...
We are testing the 2 tailed hypothesis with an H0 that that slope =0 (not linear) and an Ha that the slope =/= 0, as our test statistic is 22.51 with 14 degrees of freedom, we get a p value of 2.163-12, allowing us to reject our null hypothesis. 
...
q3 <- tibble(
  ...
  "P-value" = c(pf(55.30/(4.70/38),1,38),NA,NA)
)
q3
pf(447.1064,1,38)
4.7/38
...
surm <- lm(sur_time~clot,surg.data1)
...
### a. What is the estimate of σ from this analysis?

The estimate for $\sigma$ is `r surm$sigma` with `r surm$df[2]` degrees of freedom. 
